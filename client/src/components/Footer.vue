<template>
  <div class="w-full h-1/3 bg-main paddingTopMain paddingXMain">
    <div
      class="
        w-full
        h-full
      ...
      "
    >
      <div class="h-full flex flex-col justify-around md:py-10 py-2 sm:mr-0 mr-2 md:w-1/2 w-auto">
        <router-link :to="`/home`" @click="setPage(0)"
          ><svg
            width="143"
            height="25"
            xmlns="http://www.w3.org/2000/svg"
            class="Header_logo__3_SJs"
          >
            <path
        ...
        <div class="footerText md:py-0 py-1">
      ...
      <div class="w-2/5 h-full flex flex-col justify-around">
        <div class="font-semibold text-white justify-between xl:flex-row flex-col flex xl:items-center items-end md:text-lg sm:text-base text-sm">
          <router-link
            :to="`/${categories[index]}`"
            ...
            @click="setPage(index)"
          >
            <div class="uppercase hoverEffect">
              {{ categories[index] }}
            </div></router-link
          >
        </div>
...
.footerText {
  @apply xl:text-base lg:text-sm text-xss text-gray-500;
}
</style>
