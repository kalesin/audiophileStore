<template>
  ...
  >
    ...
    >
      ...
      >
        <router-link :to="`/home`" @click="setPage(0)"
          ...
          <img src="../assets/logo.png" width="143"
            height="25"
            class="md:mb-0 sm:mb-1 mb-2 hidden" />
        </router-link>
        <router-link
          :to="`/${categories[index]}`"
        ...
        >
          <div
            class="uppercase link xl:text-lg lg:text-base md:text-sm text-xs"
            :class="{ 'text-sec': page == index + 1 && page > 0 }"
          >
            {{ categories[index] }}
          </div></router-link
        >
    ...
    <div
      v-if="page == 0"
      class="heroContainer paddingXMain"
    >
    <Hero />
    </div>
  </div>
...
.link {
  transition: opacity, color 100ms ease-out 0ms;
}
</style>
